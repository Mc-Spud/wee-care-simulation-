...
export class PoseSystem {
  ...
  generateKeypoints(person, elapsed, breathPulse) {
    ...
    switch (pose) {
      ...
      case 'reaching':    kps = this.poseReaching(px, pz, elapsed, bp); break;
  ...
  poseReaching(px, pz, elapsed, bp) {
    const base = this.poseStanding(px, pz, elapsed, 0, bp);
    const reach = Math.sin(elapsed * 2.0) * 0.15 + 0.15;
    
    // Right arm reaching forward (+z)
    base[8][2] += 0.2 + reach;   // right elbow forward
    base[8][1] += 0.15;          // right elbow up
    base[10][2] += 0.4 + reach * 1.5; // right wrist forward
    base[10][1] += 0.3;          // right wrist up
    
    // Left arm counter-balance slightly
    base[7][2] -= 0.1;
    base[9][2] -= 0.15;
