...
export const SCENARIO_NAMES = [
  'VITAL SIGNS','MULTI-PERSON','FALL DETECT','INTRUSION DETECT','SLEEP MONITOR',
];

export const DEFAULTS = {
  bloom: 0.08, bloomRadius: 0.2, bloomThresh: 0.6,
  exposure: 1.2, vignette: 0.15, grain: 0.01, chromatic: 0.0005,
  boneThick: 0.022, jointSize: 0.045, glow: 0.2, trail: 0.4,
  wireColor: '#87c830', jointColor: '#87c830', aura: 0.04,
  field: 0.55, waves: 0.7, ambient: 0.8, reflect: 0.2,
  fov: 50, orbitSpeed: 0.15, grid: true, room: true,
  scenario: 'search_rescue', cycle: 20, dataSource: 'demo', wsUrl: '',
};
...
const SCENARIO_DESCRIPTIONS = {
  auto:              'Auto-cycling through healthcare sensing scenarios.',
  single_breathing:  'Detecting vital signs (heart & respiratory rate) through WiFi micro-variations.',
  two_walking:       'Tracking multiple persons simultaneously via CSI multiplex spatial separation.',
  fall_event:        'Sudden posture-change and emergency fall detection via acceleration analysis.',
  sleep_monitoring:  'Continuous contactless sleep tracking and breathing pattern analysis.',
};

// Edge modules active per scenario
const SCENARIO_EDGE_MODULES = {
  auto:              ['VITALS', 'TRACKING', 'FALL', 'SLEEP'],
  single_breathing:  ['VITALS', 'BREATHING'],
  two_walking:       ['TRACKING', 'MULTI-PERSON'],
  fall_event:        ['FALL DETECT', 'ALERT'],
  intrusion_detect:  ['ALERT', 'TRACKING', 'PRESENCE'],
  sleep_monitoring:  ['SLEEP', 'VITALS'],
};
...
export class HudController {
  ...
  initSettings() {
    ...
    document.getElementById('btn-export-settings').addEventListener('click', () => {
      ...
      a.download = 'WeeCare-observatory-settings.json';
      a.click();
    });
  ...
  initQuickSelect() {
    ...
    // Force inject the option if the user has a cached HTML file without it
    if (!sel.querySelector('option[value="intrusion_detect"]')) {
      const opt = document.createElement('option');
      opt.value = 'intrusion_detect';
      opt.textContent = 'Intruder Detect';
      sel.insertBefore(opt, sel.querySelector('option[value="sleep_monitoring"]'));
    }
  ...
  saveSettings() {
    try {
      localStorage.setItem('WeeCare-observatory-settings', JSON.stringify(this._obs.settings));
    } catch {}
  }
  ...
  updateHUD(data, demoData) {
    ...
    if (fallEl) {
      ...
      // Force inject intruder-alert if user has a cached HTML file without it
      let intruderEl = document.getElementById('intruder-alert');
      if (!intruderEl) {
        intruderEl = document.createElement('div');
        intruderEl.id = 'intruder-alert';
        intruderEl.className = 'intruder-alert';
        intruderEl.style.display = 'none';
        intruderEl.textContent = 'INTRUDER DETECTED';
        
        // Apply inline styles to bypass cached CSS
        intruderEl.style.marginTop = '10px';
        intruderEl.style.textAlign = 'center';
        intruderEl.style.padding = '8px';
        intruderEl.style.borderRadius = '8px';
        intruderEl.style.fontFamily = "'JetBrains Mono', monospace";
        intruderEl.style.fontSize = '12px';
        intruderEl.style.fontWeight = '700';
        intruderEl.style.letterSpacing = '2px';
        intruderEl.style.background = 'rgba(224, 128, 0, 0.15)';
        intruderEl.style.color = 'var(--amber)';
        intruderEl.style.border = '1px solid rgba(224, 128, 0, 0.4)';
        // Note: pulse-border animation might not exist in cached CSS, but static style is better than nothing
        
        fallEl.parentNode.insertBefore(intruderEl, fallEl.nextSibling);
      }
      intruderEl.style.display = cls.intrusion ? 'block' : 'none';
    }
