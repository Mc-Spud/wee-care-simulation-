...
export class DemoDataGenerator {
  ...
  _fallEvent(t) {
    ...
    return this._baseFrame({
      nodes: [{ node_id: 1, rssi_dbm: rssi, position: [-2, 0.45, -1], amplitude, subcarrier_count: 64 }],
      features: {
        mean_rssi: rssi,
  ...
  _intrusionDetect(t) {
    ...
    const doorOpen = (cycleT >= 0 && cycleT < 1) || (cycleT >= 28 && cycleT < 29);
    const entered = cycleT >= 0.5 && cycleT < 28;
    const cautiousEntry = cycleT >= 0.5 && cycleT < 4;
    const checkingCorners = cycleT >= 4 && cycleT < 8;
    const settledSearch = cycleT >= 12 && cycleT < 20;
    const loitering = cycleT >= 20 && cycleT < 24;
    const escaping = cycleT >= 24 && cycleT < 28;

    const doorPressure = doorOpen ? Math.sin((cycleT - (cycleT >= 28 ? 28 : 0)) * Math.PI) * 0.4 : 0;

    let px, pz, facing, ms, pose;
    if (!entered) {
      px = -5.5; pz = -2; facing = 0; ms = 0; pose = 'absent';
    } else if (cautiousEntry) {
      const entryT = cycleT - 0.5;
      const movePhase = entryT % 1.5;
      const isMoving = movePhase > 0.6 && movePhase < 1.3;
      const baseP = Math.floor(entryT / 1.5) * 0.5;
      const moveAmt = Math.max(0, Math.min(1, (movePhase - 0.6) / 0.7));
      const progress = Math.min(1, baseP + moveAmt * 0.5);
      
      px = _lerp(-5.5, -2, progress);
      pz = _lerp(-2, -0.5, progress);
      if (!isMoving) {
        px += Math.sin(t * 4) * 0.05;
        facing = Math.sin(t * 2) * 0.5 + 0.8;
      } else {
        facing = Math.atan2(3.5, 1.5);
      }
      ms = isMoving ? 100 : 8;
      pose = 'crouching';
    } else if (checkingCorners) {
      const cornerT = (cycleT - 4) / 4;
      const cornerIdx = Math.floor(cornerT * 3) % 3;
      const corners = [[-2, -0.5], [1.5, 1.5], [1.5, -1.5]];
      ...
      facing = inTransit ? Math.atan2(corners[(cornerIdx + 1) % 3][0] - corner[0], corners[(cornerIdx + 1) % 3][1] - corner[1]) : Math.sin(t * 3) * Math.PI;
      ms = inTransit ? 120 : 10;
      pose = 'crouching';
    } else if (cycleT < 12) {
      const checkT = (cycleT - 8) / 4;
      px = 1.5;
      pz = _lerp(1.5, -0.5, checkT);
      facing = Math.PI;
      ms = 90;
      pose = 'walking';
    } else if (settledSearch) {
      const searchT = cycleT - 12;
      const deskX = 1.5, deskZ = -0.8;
      px = deskX + (Math.sin(searchT * 2.1) * 0.2) + _harmonicNoise(t, 22.1, 2) * 0.2;
      pz = deskZ + (Math.cos(searchT * 1.5) * 0.2) + _harmonicNoise(t, 23.5, 2) * 0.2;
      ...
      px = 1.5; pz = -0.8;
      facing = Math.sin(t * 0.5) * 0.5;
      ms = 5;
      pose = 'standing';
    } else if (escaping) {
      const escapeT = cycleT - 24;
      const progress = Math.min(1, escapeT / 4);
      px = _lerp(1.5, -5.8, progress);
      pz = _lerp(-0.8, -2.5, progress);
      facing = -Math.PI / 2;
      ms = 180;
      pose = 'walking';
    } else {
      // t >= 24: escaped. Keep them standing outside the door so they don't just vanish into thin air!
      px = -5.8; pz = -2.5; 
      facing = Math.PI; 
      ms = 5; // barely moving
      pose = 'standing';
    ...
    return this._baseFrame({
      ...
      persons: entered ? [{ id: 'p0', position: [px, 0, pz], motion_score: ms, pose, facing: facing + Math.PI }] : [],
      ...
      edge_modules: {
        ...
        loitering: { detected: loitering || settledSearch, duration_s: loitering ? (cycleT - 20).toFixed(1) : (settledSearch ? (cycleT - 12).toFixed(1) : 0) },
  ...
  _searchRescue(t) {
    ...
    const cycleT = t % 12; // 12-second fast cycle
    const scanning = false;
    const falsePos1 = false;
    const scan2 = false;
    const falsePos2 = false;
    const genuineDetect = true;
    const lockingOn = cycleT < 2;
    const confirmed = cycleT >= 2;
    const stableMonitor = cycleT >= 5;

    // Scan sweep effect (nodes cycle through angles)
    const scanAngle = t * 0.8;

    const isWalking = cycleT < 4;
    const isFalling = cycleT >= 4 && cycleT < 5;
    const hasFallen = cycleT >= 5;
    ...
    if (isWalking) {
        currentPose = 'walking';
        pz = -3 + cycleT * 0.75; // Walk faster
        px = 4 + Math.sin(cycleT * 6) * 0.1;
        currentMotion = 80;
    } else if (isFalling) {
        currentPose = 'falling';
        pz = 0;
        px = 4;
        fallProg = (cycleT - 4) / 1.0; // 1 second fall
        currentMotion = 200;
    } else {
        currentPose = 'fallen';
        pz = 0;
    ...
    const targetPos = [px, 0, pz];
    const nodePositions = [[2, 0, 4], [-2, 0, 4], [0, 0, -4]];
    const nodes = [];
    ...
    // Vital sign extraction: confidence is higher when they fall and are still
    const vitalConfidence = hasFallen ? 0.9 : (isWalking ? 0.3 : 0.6);
    
    let baseHR = 75;
    let baseBR = 14;
    if (isWalking) {
        baseHR = 85 + Math.sin(t * 0.2) * 5; // Elevated while walking
        baseBR = 18 + Math.sin(t * 0.5) * 2;
    } else if (isFalling) {
        baseHR = 110 + Math.random() * 10; // Spikes during fall
        baseBR = 24 + Math.random() * 4;
    } else if (hasFallen) {
        // Elevated but stabilizing after fall
        baseHR = 95 + Math.sin(t * 1.5) * 8 + n.hr(t) * 4;
        baseBR = 20 + Math.sin(t * 2) * 3 + n.br(t) * 2;
    }

    const breathRate = genuineDetect ? baseBR : 0;
    const heartRate = genuineDetect ? baseHR : 0;
    ...
    const persons = detected && genuineDetect
      ? [{ id: 'p0', position: targetPos, motion_score: currentMotion, pose: currentPose, fallProgress: fallProg, facing: 0, signal_strength: confidence }]
    ...
    return this._baseFrame({
      ...
      classification: {
        ...
        presence: detected,
        confidence,
        fall_detected: isFalling || hasFallen,
        through_wall: true,
      ...
      vital_signs: {
        ...
        heart_rate_bpm: genuineDetect && vitalConfidence > 0.2 ? heartRate : 0,
        breathing_confidence: genuineDetect ? vitalConfidence * 0.95 : 0,
        heart_rate_confidence: genuineDetect ? vitalConfidence * 0.8 : 0,
      },
      persons,
