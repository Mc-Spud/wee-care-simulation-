...
export class ScreenshotTool {
  ...
  async captureToFile(element) {
    ...
    link.href = dataUrl;
    link.download = `WeeCare-screenshot-${this.timestamp()}.png`;
    link.click();
  }
  ...
  async captureCanvasFallback(element) {
    ...
    ctx.fillText(`WeeCare - ${new Date().toLocaleString()}`, 10, mergedCanvas.height - 10);
  ...
  async renderToCanvas(element) {
    ...
    ctx.fillText(`WeeCare - ${new Date().toLocaleString()}`, 10, rect.height - 10);
