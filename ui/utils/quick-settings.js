...
export class QuickSettings {
  ...
  createPanel() {
    ...
    this.panel.querySelector('#qs-reset-tour').addEventListener('click', () => {
      try { localStorage.removeItem('WeeCare-onboarding-done'); } catch { /* noop */ }
      this.close();
  ...
  getSetting(key) {
    try { return JSON.parse(localStorage.getItem(`WeeCare-setting-${key}`)); }
    catch { return null; }
  }

  saveSetting(key, value) {
    try { localStorage.setItem(`WeeCare-setting-${key}`, JSON.stringify(value)); }
    catch { /* noop */ }
  }
